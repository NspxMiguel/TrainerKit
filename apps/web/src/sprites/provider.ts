...
export function spriteUrl(req: SpriteRequest, settings: SpriteSettings = getSpriteSettings()): string | null {
  if (settings.source === "off") return null;
  if (req.spriteId === null) return null;

  switch (settings.source) {
    case "pokeapi-artwork":
      return `${POKEAPI}/other/official-artwork${req.shiny ? "/shiny" : ""}/${req.spriteId}.png`;

    case "pokeapi-home":
      return `${POKEAPI}/other/home${req.shiny ? "/shiny" : ""}/${req.spriteId}.png`;
