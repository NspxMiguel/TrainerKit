  ...
  return createPortal(
    <div className="tk-sheet-full" role="dialog" aria-modal="true" aria-label={species.name} data-saindo={saindo || undefined}>
      ...
      */}
      ...
      {/*
        ⚠️ AQUI HAVIA UMA SEGUNDA BOLHA DE IA, e ela saiu.

        Eram DUAS bolhas flutuantes no mesmo canto desta mesma tela: esta, que
        so ROLAVA ate o cartao do assistente, e a `AiBubble` la embaixo, que
        abre a conversa de verdade. Medi as duas no DOM — 54px em `bottom: 645`
        e 52px em `bottom: 734`, uma logo acima da outra, as duas violeta, as
        duas sobre IA.

        Duas portas para a mesma coisa nao sao duas chances: sao uma duvida. E a
        que ficou e a que FAZ mais — o cartao do assistente continua no fluxo,
        entao o atalho pra ele nunca foi necessario.

        O handoff pede UMA bolha ("54px de vidro, canto inferior direito,
        estrela em gradiente violeta"), e a `AiBubble` e ela.
      */}
