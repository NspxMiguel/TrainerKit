...
import { ACTION_LABELS, decide, type Action } from "@trainerkit/core";
          ...
          {!pendencias ? (
            ...
            <section className="tk-card">
              <div style={{ font: "700 15px var(--tk-font)" }}>
                {pendencias.total === 1
                  ? "1 Pokémon salvo, nada pendente"
                  : `${pendencias.total} Pokémon salvos, nada pendente`}
              </div>
              <p className="tk-caption" style={{ marginTop: 6, lineHeight: 1.5 }}>
                Todos com veredito de guardar. Escaneie mais prints e eu aviso
                quando algum pedir ação.
              </p>
            </section>
          ) : (
            <section className="tk-card" style={{ display: "grid", gap: 12 }}>
              <div>
                <div style={{ font: "700 15px var(--tk-font)" }}>
                  {pendencias.agir.length === 1
                    ? "1 pede uma decisão"
                    : `${pendencias.agir.length} pedem uma decisão`}
                </div>
                <p className="tk-caption" style={{ marginTop: 4 }}>
                  de {pendencias.total} {pendencias.total === 1 ? "salvo" : "salvos"}
                </p>
              </div>
              ...
              {pendencias.agir.slice(0, 3).map((d) => (
                ...
                >
                  ...
                  >
                    {ACTION_LABELS[d.verdict.action]}
                  </span>
                </div>
              ))}
              ...
              {pendencias.agir.length > 3 && (
                <p className="tk-caption">
                  e mais {pendencias.agir.length - 3} na aba Coleção.
                </p>
              )}
            </section>
          )}
