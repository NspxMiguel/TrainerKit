...
export default function Ficha() {
   ...
   */
  const salvo = useMemo(() => (itens ?? []).find((g) => g.speciesId === id) ?? null, [itens, id]);
          ...
          */}
                    ...
                    {rastro &&
                      veredito.signals.map((sinal, i) => (
                        <View
                          ...
                          style={
                            i > 0 ? { borderTopWidth: 1, borderTopColor: cores.linha } : undefined
                          }
                        >
          ...
          em que se joga, e a única que cruza o IV com a coleção. */}
            {salvo && !salvo.ivDesconhecido && dados && (
              <EntreOsSeus especie={especie} ivs={salvo.ivs} todas={dados.species} jaSalvo />
            )}
          ...
          "Evoluir agora"). Branco ele era só mais um botão. */}
            <Link href={{ pathname: "/iv/[id]", params: { id: especie.id } }} asChild>
              <Toque
                ...
                style={{
                  backgroundColor: veredito
                    ? cores[COR_ACAO[veredito.action] ?? "texto"]
                    : cores.texto,
      ...
      */}
            {salvo && (
              <Toque
                ...
                style={{
                  borderWidth: 1,
                  borderColor: confirmandoTirar ? cores.transferir : cores.linha,
                }}
              >
                <Text
                  ...
                  <View className="flex-row items-center justify-between mt-3">
                    ...
                    custa subir a especie inteira, que e a pergunta real. */}
                      {t("species.maxCostValue", {
                        candy:
                          max.custo.ataque.doces +
                          max.custo.guarda.doces +
                          max.custo.espirito.doces,
                        xl:
              ...
              <View className="bg-superficie rounded-cartao p-5 mt-3">
                ...
                {especie.evolvesInto.map((idEvo, i) => {
                  ...
                  return (
                    <Link
                      key={idEvo}
                      href={{ pathname: "/especie/[id]", params: { id: idEvo } }}
                      asChild
                    >
                      <Pressable
                      ...
                      >
                        <Text className="text-texto text-[15px] flex-1">
                          {alvoEvo?.name ?? idEvo}
                        </Text>
                        {doces !== null && (
        ...
        >
          {aberto && (
            <Vidro raio={26} style={{ padding: 18, marginBottom: 12 }}>
              ...
              />
              {pensando && (
                <Text className="text-texto3 text-legenda mt-3">{t("ai.thinking")}</Text>
              )}
              {resposta && !pensando && (
