...
/*
 * ⚠️ KV-STORE, E NAO DEXIE. No web a copia guardada mora em IndexedDB; aqui
 * IndexedDB nao existe. `expo-sqlite/kv-store` e a mesma ideia — chave e valor,
 * sincrono na leitura — sobre SQLite, que ja e dependencia do app.
 *
 * O feed inteiro cabe num valor (30 kB de eventos, 19 kB de ovos), entao nao
 * vale montar tabela: um `getItem`/`setItem` por feed diz a mesma coisa e cabe
 * em dez linhas.
 */

async function guardar(nome: FeedGuardado["nome"], dados: unknown): Promise<void> {
  try {
    await AsyncStorage.setItem(`tk:agenda:${nome}`, JSON.stringify({ nome, dados, em: Date.now() }));
  } catch {
    // Sem disco a tela ainda funciona com o que veio da rede.
  }
}

async function guardado(nome: FeedGuardado["nome"]): Promise<FeedGuardado | null> {
  try {
    const cru = await AsyncStorage.getItem(`tk:agenda:${nome}`);
    return cru ? (JSON.parse(cru) as FeedGuardado) : null;
  } catch {
    return null;
  }
}
